// ============================================================
// PHONE NUMBER VALIDATION ENGINE v2 - Google Apps Script
// ============================================================
...
function fireBatch(forceSlot) {
  ...
  var currentSlot = forceSlot || getCurrentSlot();
  if (!currentSlot) {
  ...
  for (var i = 0; i < data.length && fired < config.batchSize; i++) {
    ...
    Utilities.sleep(750);
    // Extra pause every `concurrent` calls
    if (fired % config.concurrent === 0 && fired > 0) {
...
function pollResults() {
  ...
  for (var i = 0; i < data.length; i++) {
    ...
    var recordingUrl = callData.recordingUrl || callData.artifact?.recordingUrl || "";
...
function classifyCall(callData) {
  ...
  var transcript = callData.transcript || callData.artifact?.transcript || "";
  var duration = callData.duration || 0;
  var messages = callData.messages || callData.artifact?.messages || [];
...
function getTranscriptPreview(callData) {
  ...
  var t = callData.transcript || callData.artifact?.transcript || "";
